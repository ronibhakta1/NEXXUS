...
export const PostEchos = () => {
    ...
    async function generateRecommendations() {
        if (!echos.content || echos.content.trim().length < 2 || echos.content.trim().length > 400) {
            toast({
                title: "Invalid input",
        ...
        try {
            const response = await axios.post(`${BACKEND_URL}/api/v1/echo/genaratesuggestionsgimini`, echos, {
                headers: {
                    authorization: localStorage.getItem('token')
                }
            });
                        ...
                        <div className="flex col-span-3 space-x-2">
                            <Dialog open={open} onOpenChange={setOpen}>
                                <DialogTrigger asChild>
                                    <button onClick={generateRecommendations} disabled={!echos.content || echos.content.trim().length < 2 || echos.content.trim().length > 400}>
                                        <BrainCircuit />
                                    </button>
                                </DialogTrigger>
